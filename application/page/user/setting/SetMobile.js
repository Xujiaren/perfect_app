...
import { Text, View ,StyleSheet,TouchableWithoutFeedback,TouchableOpacity,ScrollView,Keyboard,TextInput} from 'react-native';
...
class SetMobile extends Component {
    ...
    _onInput(text) {
        ...
        // if( code.length === 5){
        //     navigation.navigate('SetPwds', {type: 1, mobile: mobile, code: text});
        // }
        ...
        if( code.length === 5){
            ...
            actions.passport.validcode({
                ...
                code: text,
                resolved: (data) => {
                    navigation.navigate('SetPwds', {type: 1});
                },
                rejected: (msg) => {
                    ...
                    <ScrollView 
                        ...
                        <View>
                            <View style={[styles.mt_25,styles.ml_25,styles.fd_r,styles.ai_ct]}>
                                <Text style={[styles.gray_label,styles.default_label, styles.mr_10]}>{step === 0 ? '你绑定的手机号为' : '已发送至绑定手机'} {mobile.replace(/(\d{3})\d{4}(\d{4})/, '$1****$2')}</Text>
...
export const LayoutComponent = SetMobile;
