...
class MsgCourse extends Component {
    ...
    componentWillReceiveProps(nextProps){
        ...
        const {usermsgcourse,navigation} = nextProps;

        if (usermsgcourse !== this.props.usermsgcourse){
            this.items = msgItems.concat(usermsgcourse.items);

            this.setState({
                msgItems : this.items
            })

			this.page = usermsgcourse.page;
            this.totalPage = usermsgcourse.pages;
    ...
    _onHeaderRefresh(){
        ...
        actions.message.usermsgcourse(this.page);
            ...
            >
                <View style={styles.head}>
                    <View style={styles.msgImgbox}>
                        <Image source={asset.reminIcon} style={styles.headCover} />
                        {
                            msg.status == 0 ?
                            <View style={styles.headDot}></View>
                        :null}
                    </View>
                    <Text style={msg.status === 0 ? [styles.lg_label,styles.black_label,styles.fw_label] : [styles.default_label,styles.tip_label] }>{msg.title}</Text>
                </View>
                <View style={[styles.d_flex ,styles.fd_c ,styles.pb_10]}>
                    <Text   style= {msg.status === 0 ? [styles.gray_label,styles.default_label,styles.lh20_label,styles.mt_5] : [styles.tip_label,styles.default_label,styles.lh20_label,styles.mt_5]}>{msg.content}&gt;&gt;</Text>
    ...
    _onMsgList(msg,index){
        ...
        navigation.navigate('MsgDesc',{msg:msg});
    }
...
export const LayoutComponent = MsgCourse;
