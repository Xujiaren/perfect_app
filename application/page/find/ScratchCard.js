    ...
    render() {
        ...
        return (
            <View style={[styles.container]} {...this._panResponder.panHandlers} ref={'test'}>
                ...
                {
                    Object.keys(this.state.showIndexs).length > 0 && Object.keys(this.state.showIndexs).map((i) => {
                        const o = this.touchCoordinates[i]            
                        return(
                            <View key={`show_${i}`} pointerEvents={'none'} style={[styles.img, {left: o.x - 6, top: o.y - 6}]} >
                            </View>
                        )
                    })
                }
...
export const LayoutComponent = ScratchCard;
