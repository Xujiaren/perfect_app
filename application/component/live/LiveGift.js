...
class LiveGift extends Component {
    ...
    push(msg, gift) {
        this.setState({
            ...
            this.refs.live_gift.bounceInLeft(800).then(state => {
                this.refs.live_gift.fadeOutUp(250).then(state => {
                    this.setState({
                        msg: '',
                        gift: {},
                    });
            ...
            <Animatable.View useNativeDriver style={styles.container} animation={'bounceInLeft'} ref={'live_gift'}>
                ...
                <LinearGradient colors={['#F2709C', '#FF9472']}  start={{x: 0, y: 0}}  end={{x: 1, y: 0}}
                    ...
                    <View style={[styles.fd_c,styles.col_1]}>
                        <Text style={[styles.white_label, styles.sm_label, styles.gift_msg]}>{this.state.msg}  </Text>
                        <Text style={[styles.white_label, styles.sm_label, styles.gift_msg]}>{'赠送'}<Text style={[styles.yellow_label, styles.sm_label]}>{gift.giftName}</Text></Text>
                    </View>
