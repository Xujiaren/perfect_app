...
class Emoji extends Component {
    ...
    render() {
        ...
        return (
            <View style={[style, styles.bg_white, styles.row, styles.f_wrap]} onLayout={(evt) => {
                this.setState({
                    ...
                    Object.keys(emojis).map((key, index) => {
                        return (
                            <TouchableOpacity style={[{width: width, height: width * 0.8}, styles.jc_ct, styles.ai_ct]} key={'emoji_' + index} onPress={() => {
                                ...
                                <Image source={{uri: 'https://edu-uat.oss-cn-shenzhen.aliyuncs.com/app/emo/' + emojis[key] + ".gif"}} style={styles.eicon}/>
                            </TouchableOpacity>
                        )
                    })
                }
